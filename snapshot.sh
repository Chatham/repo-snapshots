...
[[ -z "${REPO_BASE}" ]] && REPO_BASE="$( getent passwd "apt-mirror" | cut -d: -f6 )/mirror"
...
function configure_apache24() {
    ...
    local conf1=10-repo-snapshots

    if [[ $dryrun -eq 0 ]]; then
        # empty the config
        >${conf_base}/${conf1}.conf
    fi
    ...
    while [[ ${#URLS[@]} -gt 1 ]]; do
        ...
        if [[ -n "${path_full}" ]]; then
            ...
            vecho "URL map /${URL_BASE}/${url_part} to ${path_full}"
            if [[ $dryrun -eq 0 ]]; then
                cat >>${conf_base}/${conf1}.conf << EOF
Alias /${URL_BASE}/${url_part} ${path_full}
EOF
            fi
        fi
    done
    ...
    do
        ...
        m)
            ...
            if [[ "${OPTARG}" == "serve" ]]; then
               mode=$serve_mode
               RUN_AS="root"
               # a2enconf may not be on default path
               PATH="${PATH}:/usr/sbin"
